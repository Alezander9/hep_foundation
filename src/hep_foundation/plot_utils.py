...
def plot_combined_training_histories(
    ...
    metric_labels: Dict[str, str] = {'loss': 'Train Loss', 'val_loss': 'Validation Loss'},
    ...
    def transform_history(epoch_history: dict) -> dict:
        ...
        transformed = {k: v for k, v in transformed.items() if not all(x is None for x in v)}
        return transformed
    ...
    try:
        ...
        line_styles = {metrics_to_plot[i]: ('-' if i % 2 == 0 else '--') for i in range(len(metrics_to_plot))} 

        for idx, (model_label, history) in enumerate(transformed_histories.items()):
            if not history:
                logging.warning(f"Skipping plot for '{model_label}' due to empty history.")
                continue
            ...
            if not any(epochs): # Check if epochs list is still empty
                 logging.warning(f"Skipping plot for '{model_label}' as no metrics to plot were found in history.")
                 continue

            for metric in metrics_to_plot:
                 if metric in history and history[metric]: # Ensure metric exists and has data
                    display_label = metric_labels.get(metric, metric) # Use provided label or default to key
        ...
        ax.grid(True, which='both', linestyle='--', linewidth=LINE_WIDTHS['thin'], alpha=0.6)
