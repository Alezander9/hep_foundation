...
from hep_foundation.config.new_task_config import NewProcessedEvent, EventSelectionData
...
class PhysliteEventProcessor:
    ...
    def _process_selection_config_new_format(
        ...
        # For normal dataset creation: check if we have valid data
        if not need_more_zero_bias_samples and not event_raw_selector_data and not any(event_aggregators_pass_filters):
            self.logger.debug("No scalar features and no valid aggregated features found")
            return None
