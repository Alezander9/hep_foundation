...
class PipelineConfigLoader:
    ...
    def _process_python_extensions(self, content: str) -> str:
        ...
        def replace_extension(match):
            ...
            try:
                ...
                self.logger.error(f"Failed to evaluate Python extension '{expression}': {e}")
    ...
    def create_config_objects(self, config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        dataset_config = self._create_dataset_config(config_dict['dataset'], task_config)
        
        # Create TrainingConfigs
        vae_training_config = self._create_training_config(config_dict['training']['vae'])
        dnn_training_config = self._create_training_config(config_dict['training']['dnn'])
        
        # Create EvaluationConfig
        evaluation_config = self._create_evaluation_config(config_dict.get('evaluation', {}))
    ...
    def _create_dataset_config(self, dataset_dict: Dict[str, Any], task_config: TaskConfig) -> DatasetConfig:
    ...
    def _create_evaluation_config(self, evaluation_dict: Dict[str, Any]) -> EvaluationConfig:
        """Create EvaluationConfig from dictionary."""
        return EvaluationConfig(
            regression_data_sizes=evaluation_dict.get('regression_data_sizes', [1000, 2000, 5000]),
            fixed_epochs=evaluation_dict.get('fixed_epochs', 10),
            anomaly_eval_batch_size=evaluation_dict.get('anomaly_eval_batch_size', 1024),
        )
    ...
    def save_config_to_registry(self, config_file_path: Path, registry_dir: Path) -> Path:
        """
