...
class EventSelectionData:
    ...
    def __init__(
        ...
        aggregator_configs: list[Any],  # References to original aggregator configs for metadata
    ):
        """
        ...
        self.event_aggregators_sort_filter_indices = event_aggregators_sort_filter_indices
    ...
    def get_aggregator_valid_count(self, aggregator_idx: int) -> int:
        """Get number of valid elements for a specific aggregator before filtering."""
        if aggregator_idx >= len(self.event_raw_aggregators_data):
            return 0
        
        # Get the count from the first branch in this aggregator
        aggregator_data = self.event_raw_aggregators_data[aggregator_idx]
        if not aggregator_data:
            return 0
        
        first_branch_data = next(iter(aggregator_data.values()))
        return len(first_branch_data)
    ...
    def get_zero_bias_hist_data(self) -> dict[str, list[float]]:
        ...
        for aggregator_idx, aggregator_data in enumerate(self.event_raw_aggregators_data):
    ...
    def get_post_selection_hist_data(self) -> dict[str, list[float]]:
        ...
        for aggregator_idx, aggregator_data in enumerate(self.event_raw_aggregators_data):
            ...
            filtered_indices = self.event_aggregators_sort_filter_indices[aggregator_idx]
    ...
    def get_ML_training_data(self) -> dict[str, np.ndarray]:
        ...
        for aggregator_idx, aggregator_data in enumerate(self.event_raw_aggregators_data):
            ...
            feature_arrays = []
            for branch_name in sorted(aggregator_data.keys()):  # Ensure consistent ordering
...
class NewProcessedEvent:
    ...
    def get_num_tracks_for_plot(self) -> Optional[int]:
        ...
        for aggregator_idx in range(len(self.input_selection_data.event_raw_aggregators_data)):
            if self.input_selection_data.event_aggregators_pass_filters[aggregator_idx]:
                filtered_count = self.input_selection_data.get_aggregator_filtered_count(aggregator_idx)
                
                # Apply max_length truncation for plotting
                aggregator_config = self.input_selection_data.aggregator_configs[aggregator_idx]
                max_length = aggregator_config.max_length
...
# Backward compatibility helpers (to be removed in step 4)
class NewTaskConfig:
    """
    Placeholder class name for the new processed event system.
    
    This will be renamed to ProcessedEvent in the final step.
    """
    pass
