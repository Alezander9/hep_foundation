        ...
        if "dropout_rate" in self.hyperparameters:
            ...
            if not isinstance(dropout_rate, (float, type(None))) or \
               (isinstance(dropout_rate, float) and not 0 <= dropout_rate <= 1):
                raise ValueError("dropout_rate must be a float between 0 and 1, or None")
        ...
        if "l2_regularization" in self.hyperparameters:
            ...
            if not isinstance(l2_reg, (float, type(None))) or \
               (isinstance(l2_reg, float) and l2_reg < 0):
                raise ValueError("l2_regularization must be a non-negative float or None")
        ...
        for i, units in enumerate(self.hidden_layers):
            ...
            if self.quant_bits:
                x = QDense(
                    ...
                    kernel_regularizer=keras.regularizers.l2(self.l2_regularization) if self.l2_regularization else None,
                    name=f'dense_{i}'
                )(x)
                x = QActivation(
