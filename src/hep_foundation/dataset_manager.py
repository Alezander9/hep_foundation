    ...
    def _create_atlas_dataset(self, dataset_config: DatasetConfig, plot_distributions: bool = False, delete_catalogs: bool = True) -> Tuple[str, Path]:
        ...
        try:
            ...
            with h5py.File(output_path, 'w') as f:
                ...
                norm_params = self.feature_processor._compute_dataset_normalization(all_inputs, all_labels if all_labels else None)
        ...
        try:
            ...
            with h5py.File(output_path, 'w') as f:
                ...
                for signal_key in dataset_config.signal_keys:
                    ...
                    # Compute and store normalization parameters
                    norm_params = self.feature_processor._compute_dataset_normalization(inputs, labels if labels else None)
    ...
    def load_atlas_datasets(self, 
        ...
        try:
            ...
            if not self.current_dataset_path.exists():
                self.current_dataset_id, self.current_dataset_path = self._create_atlas_dataset(dataset_config, delete_catalogs=delete_catalogs)
        ...
        try:
            ...
            with h5py.File(dataset_path, 'r') as f:
                for signal_key in f.keys():
                    ...
                    features_dict = self.feature_processor._load_features_from_group(signal_group['features'])
                    labels_dict = None
                    if include_labels and 'labels' in signal_group and signal_group.attrs.get('has_labels', False):
                        labels_dict = self.feature_processor._load_labels_from_group(signal_group['labels'])
