...
try:
    ...
    logging.warning(
        "PhysLite branch index not found. Branch validation will be limited."
    )
    PHYSLITE_BRANCHES = {}
...
def get_branch_info(
    ...
    is_valid = (
        branch_info.get("verified", False)
        or branch_info.get("shape") is not None
        or branch_info.get("status") == "success"
    )
