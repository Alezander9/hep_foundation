...
def make_request(url: str, run_number: str, max_retries: int = 3, initial_delay: float = 1.0) -> requests.Response:
    """
...
def validate_catalog_count(run_number: str, count: int) -> bool:
    ...
    padded_run = run_number.zfill(8)
    url_exists = f"/record/80001/files/data16_13TeV_Run_{padded_run}_file_index.json_{count-1}"
    url_not_exists = f"/record/80001/files/data16_13TeV_Run_{padded_run}_file_index.json_{count}"
...
def verify_catalog_counts(catalog_data: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    """
    ...
    for run_number, data in list(catalog_data.items()):
        ...
        try:
            ...
            if validate_catalog_count(run_number, count):
                verifications += 1
                print(f"Verification succeeded (total successes: {verifications}, failures: {failures})")
            else:
                failures += 1
                print(f"Verification failed (total successes: {verifications}, failures: {failures})")
            ...
            if failures > verifications:
                print(f"Run {run_number} has more failures than successes, marking for recalculation")
...
def generate_catalog_counts():
    ...
    # Convert all run numbers to strings
    run_numbers = [str(run) for run in ATLAS_RUN_NUMBERS]
