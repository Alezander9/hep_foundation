...
def main():
        ...
        for i, gpu in enumerate(gpus):
            ...
            try:
                ...
                if details and 'name' in details: # Check if 'name' key exists
                    ...
                    print(f"    Name: Not available (PhysicalDevice: {gpu.name})") # Fallback if name is not in details
    ...
    if cuda_available and gpus:
        print("CUDA should be available for TensorFlow operations on the detected GPU(s).")
        ...
        print("Warning: GPUs detected, but TensorFlow is not built with CUDA. GPU acceleration will not be available.")
    else:
