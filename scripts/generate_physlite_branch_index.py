...
def get_branch_info(file_path: Union[str, Path], max_entries: int = 100) -> Dict[str, Dict[str, Dict[str, Any]]]:
    """
    ...
    with uproot.open(file_path) as file:
        ...
        for branch in all_branch_names:
            ...
            if '.' in branch:
                ...
                branch_info[category][feature] = {'shape': None, 'dtype': None, 'status': 'unread'}
            else:
                # Handle branches without dots
                if 'Other' not in branch_info:
                    branch_info['Other'] = {}
                branch_info['Other'][branch] = {'shape': None, 'dtype': None, 'status': 'unread'}
                    ...
                    if full_branch in arrays:
                        ...
                        if len(sample) > 0:
                            ...
                            if hasattr(first_entry, 'shape'):
                                ...
                                item_type = type(first_entry[0]).__name__ if first_entry else 'unknown'
                            else:
                                # For scalars
                                shape = ()
        ...
        successful_branches = sum(
            1 for cat in branch_info for feat in branch_info[cat] 
                ...
                if len(samples) > 0:
                    ...
                    for i, sample in enumerate(samples):
                        ...
                        if hasattr(sample, 'shape'):
                            lengths.append(sample.shape[0] if len(sample.shape) > 0 else 1)
                        ...
                        if i < 3:  # Only store a few samples
                            info['sample_values'].append(str(sample)[:100] + '...' if len(str(sample)) > 100 else str(sample))
                    ...
                    if hasattr(first_entry, 'shape'):
                        ...
                        info['dtype'] = type(first_entry[0]).__name__ if first_entry else 'unknown'
                    else:
                        info['shape'] = ()
...
def save_branch_dictionary(run_number: str, 
                          catalog_index: int = 0, 
    ...
    try:
        ...
        with open(output_path, 'w') as f:
            ...
            f.write(f"# Success rate: {successful_branches}/{total_branches} branches ({successful_branches/total_branches:.1%})\n\n")
